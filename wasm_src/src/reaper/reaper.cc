...
static void convert_f32_to_s16(int16_t* pOut, const float* pIn, size_t sampleCount)
{
    int r;
    size_t i;
...
{
    // Convert f32 audio to s16 audio.
    uint64_t sampleCount = length;
    std::vector<int16_t> convAudio(sampleCount);
    convert_f32_to_s16(convAudio.data(), audio, sampleCount);
