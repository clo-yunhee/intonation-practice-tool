...
std::vector<std::array<double, 6>> Butterworth::lowPass(int N, double fc, double fs)
{
...
std::vector<std::array<double, 6>> Butterworth::highPass(int N, double fc, double fs)
{
    const double Wn = fc / (fs / 2.0);
    const double Wo = std::tan(Wn * PI / 2.0);
